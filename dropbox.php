...
class Dropbox
{
     ...
     */
    public function __construct($accessTokenOrAppCredentials = null, int $maxChunkSize = self::MAX_CHUNK_SIZE, int $maxUploadChunkRetries = 0)
    {
...
// var_dump($arguments);
        $response = $this->contentEndpointRequest('files/download_zip', $arguments, null);
        return $response;
        // return StreamWrapper::getResource($response);
    }
    ...
    public function getThumbnail(string $path, string $format = 'jpeg', string $size = 'w64h64'): string
    {
        ...
        $response = $this->contentEndpointRequest('files/get_thumbnail', $arguments, null);
    ...
    public function uploadSessionStart($contents, bool $close = false): UploadSessionCursor
    {
        ...
        $response = json_decode(
            $this->contentEndpointRequest('files/upload_session/start', $arguments, $contents),
            true
        );
    ...
    public function contentEndpointRequest(string $endpoint, array $arguments, $body = '')
    {
        ...
        try {
            $response = $this->post($this->getEndpointUrl('content', $endpoint), [
    ...
    public function rpcEndpointRequest(string $endpoint, array $parameters = null): array
    {
        try {
            ...
            $response = $this->post($this->getEndpointUrl('api', $endpoint), $options);
        } catch (ClientException $exception) {
            throw $this->determineException($exception);
        }
    ...
    private function request($method, $uri, array $options = []){
       ...
       if(!empty($options['body'])) curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($options['body']));
        curl_setopt($ch, CURLOPT_HTTPHEADER, $this->_build_headers($options));
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);

        # Remove these if you are not using localhost
    ...
    private function _build_headers(array $headers){
        $header = [];
        foreach($headers['headers'] as $key => $entry){
           $header[] = "$key:$entry"; 
        } 
        if($headers['body'] !== null) array_push($header, "Content-Type: application/json");
        return $header;
    }
}
